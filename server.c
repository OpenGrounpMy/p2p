...
void TransmitMsg(int cmd,int locate,MESSAGE msg)
{
    ...
    if(cmd==PRIVATE)
    {
        if(clientList[msg.sendUserLocate].chatnow==1||msg.clientList[msg.sendUserLocate].chatnow==1)
        {
            strcpy(msg.message,"抱歉，用户正忙");
            write(clientList[locate].socketFd,&msg,sizeof(msg));
        }else{
            write(clientList[msg.sendUserLocate].socketFd,&msg,sizeof(msg));
            printf("\e[31m#聊天 >  %-5s向%-5s发送聊天邀请\n",clientList[locate].name,clientList[msg.sendUserLocate].name);
        }
    }else{
        int i;
        for (i=0;i<MAX_CLIENT;i++)
...
int main()
{
    ...
    pthread_create(&tid1,NULL,SendMsg,NULL);//创建线程
    ...
    if(-1== (fd=socket(AF_INET,SOCK_STREAM,0)))//udp套接字
    {
        perror("socket error ");
        exit(1);
    }
